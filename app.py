...
def api():
    ...
    # Prepare full context (truncate to stay within GPT limits)
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system", "content": "You are a helpful assistant answering questions based only on the context provided."},
    ...
    # Find best matching context entries
    matched_links = []
    answer_lower = answer.lower()
    for entry in CONTEXT_ENTRIES:
        text_snippet = entry["text"][:300]
        if any(word in answer_lower for word in text_snippet.lower().split()):
    ...
    if not matched_links:
        matched_links = [
            ...
            {"url": FALLBACK_DISCOURSE_URL, "text": "Discourse"}
        ]
